// components/generate/ProjectForm.jsx
// PURPOSE: The input form where users enter skills, domain, difficulty
//          and available time before triggering AI generation.
...
export default function ProjectForm({ onSubmit, isLoading }) {
  const [form, setForm] = useState({ skills: '', domain: '', difficulty: 'Intermediate' });
  ...
  const handleSubmit = (e) => { e.preventDefault(); if (!form.skills || !form.domain || totalHours < 1) return; onSubmit({ ...form, time_hours: totalHours }); };
  ...
  return (
    <div className="max-w-2xl mx-auto">
      <div className="text-center mb-12 animate-fade-in-up opacity-0">
        ...
        <p className="text-gray-400 text-lg">Tell us about yourself and we'll generate tailored project ideas.</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="glass-card p-8 space-y-3 animate-fade-in-up opacity-0 animate-delay-100">
          <label htmlFor="skills" className="flex items-center gap-2.5 text-sm font-bold text-indigo-300 uppercase tracking-widest">
            <Code2 className="w-4 h-4" /> Your Expertise
          </label>
          <input id="skills" type="text" value={form.skills} onChange={(e) => handleChange('skills', e.target.value)}
        ...
        <div className="glass-card p-8 space-y-3 animate-fade-in-up opacity-0 animate-delay-200">
          <label htmlFor="domain" className="flex items-center gap-2.5 text-sm font-bold text-purple-300 uppercase tracking-widest">
            <Globe className="w-4 h-4" /> Domain of Interest
          </label>
          <input id="domain" type="text" value={form.domain} onChange={(e) => handleChange('domain', e.target.value)}
        ...
        <div className="glass-card p-8 space-y-4 animate-fade-in-up opacity-0 animate-delay-300">
          <label className="flex items-center gap-2.5 text-sm font-bold text-indigo-300 uppercase tracking-widest">
            <BarChart3 className="w-4 h-4" /> Complexity Level
          </label>
          <div className="flex gap-4">
            {difficulties.map((level) => (
              <button type="button" key={level} onClick={() => handleChange('difficulty', level)}
                className={`flex-1 py-3.5 rounded-2xl text-sm font-bold transition-all duration-500 border ${form.difficulty === level ? 'bg-indigo-500/20 border-indigo-500/50 text-white shadow-[0_0_20px_rgba(99,102,241,0.2)]' : 'bg-white/[0.02] border-white/5 text-gray-500 hover:border-white/20 hover:text-gray-300'}`}>
                {level}
              </button>
            ))}
          </div>
        ...
        <div className="glass-card p-8 space-y-4 animate-fade-in-up opacity-0 animate-delay-400">
          ...
          <div className="grid grid-cols-3 gap-3">
            {[
              { id: 'time-weeks', val: weeks, set: setWeeks, max: 52, label: 'Weeks' },
              { id: 'time-days',  val: days,  set: setDays,  max: 4,  label: 'Days'  },
              { id: 'time-hours', val: hours, set: setHours, max: 7,  label: 'Hours' },
            ].map(({ id, val, set, max, label }) => (
              <div key={id} className="flex flex-col items-center gap-2">
                <input id={id} type="number" min="0" max={max} value={val || ''} aria-label={label}
                  onChange={(e) => set(Math.max(0, Math.min(max, Number(e.target.value) || 0)))} placeholder="0"
                ...
                <label htmlFor={id} className="text-xs font-black text-gray-500 uppercase tracking-widest">{label}</label>
              </div>
            ))}
          </div>
          <p className="text-xs text-gray-500 font-medium">
            {preview ? <span>Total: <span className="text-indigo-400 font-bold">{preview}</span> &middot; {totalHours} working hours</span> : 'Enter your available time above'}
          </p>
        </div>
        ...
        <div className="pt-4 animate-fade-in-up opacity-0 animate-delay-500">
          <button type="submit" disabled={!isValid || isLoading}
            className={`w-full btn-primary py-5 rounded-2xl text-lg font-bold shadow-2xl transition-all ${!isValid || isLoading ? 'opacity-40 cursor-not-allowed grayscale' : 'hover:scale-[1.02] active:scale-[0.98]'}`}>
            <span className="flex items-center justify-center gap-3">
              {isLoading ? <div className="w-5 h-5 border-2 border-white/30 border-t-white rounded-full animate-spin" /> : <Sparkles className="w-6 h-6" />}
